...
export class Minimap {
  ...
  update() {
    ...
    this.minimapCanv.style.translate =
      GameVars.gameW -
      this.minimapCanv.width -
      toPixelSize(12) +
      "px " +
      toPixelSize(24) +
      "px";
    ...
    !this.visited[
      GameVars.currentRoom.roomY + "|" + GameVars.currentRoom.roomX
    ] &&
      (this.visited[
        GameVars.currentRoom.roomY + "|" + GameVars.currentRoom.roomX
      ] = true);
    ...
    this.minimapCanv
      .getContext("2d")
      .clearRect(0, 0, this.minimapCanv.width, this.minimapCanv.height);
    ...
    for (let y = -2; y <= 2; y++) {
      for (let x = -2; x <= 2; x++) {
        ...
        if (
          this.validatePos(
            GameVars.currentRoom.roomX + x,
            GameVars.currentRoom.roomY + y,
          )
        ) {
          if (
            this.visited[
            ...
            GameVars.gameBoard.board[GameVars.currentRoom.roomY + y][
              GameVars.currentRoom.roomX + x
            ] &&
            this.containsVisitedBlockArround(
              GameVars.currentRoom.roomX + x,
              GameVars.currentRoom.roomY + y,
            )
          ) {
  ...
  validatePos(x, y) {
    return (
      y >= 0 &&
      y < GameVars.gameBoard.board.length &&
      x >= 0 &&
      x < GameVars.gameBoard.board[0].length
    );
  }
