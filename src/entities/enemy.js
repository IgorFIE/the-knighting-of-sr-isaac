...
import {
  EnemySubType,
  EnemyType,
  enemyChainMailColors,
} from "../enums/enemy-type";
...
import {
  ...
  rectCircleCollision,
} from "../utilities/collision-utilities";
import {
  createElem,
  drawSprite,
  setElemSize,
...
export class Enemy {
  constructor(room, x, y, enemyType, enemySubType) {
    ...
    this.enemySubType =
      enemySubType ||
      enemyChainMailColors[
        enemyType === EnemyType.BASIC ? randomNumb(3) : randomNumbOnRange(1, 2)
      ];
    ...
    this.lifeBar = new LifeBar(
      this.getEnemyLife() * GameVars.heartLifeVal,
      false,
  ...
  init(x, y) {
    ...
    this.collisionObj = new CircleObject(
      x,
      y,
      toPixelSize(this.enemySize * 2.5),
    );
    ...
    this.targetPos = new CircleObject(
      this.collisionObj.x,
      this.collisionObj.y,
      this.collisionObj.r,
    );
    ...
    setElemSize(
      this.shadowCanv,
      toPixelSize(this.enemySize) * 7,
      toPixelSize(this.enemySize) * 6,
    );
    this.shadowCanv.style.translate =
      -toPixelSize(this.enemySize * 2) +
      "px " +
      toPixelSize(this.enemySize * 4) +
      "px";
  ...
  setEnemyWeapons() {
    if (this.enemyRightWeapon) {
      ...
      const maxValue =
        GameVars.gameLevel + 5 >= Object.keys(WeaponType).length
          ? Object.keys(WeaponType).length - 1
          : GameVars.gameLevel + 5;
      const isLeftWeapon = randomNumb(2) === 0;
      if (GameVars.gameLevel < 4 && this.enemyType !== EnemyType.BOSS) {
        this.enemyRightWeapon = new Weapon(
          isLeftWeapon ? WeaponType.FIST : randomNumbOnRange(0, maxValue),
          -1,
          this,
          "#3e3846",
          this.enemySize,
        );
        this.enemyLeftWeapon = new Weapon(
          isLeftWeapon ? randomNumbOnRange(0, maxValue) : WeaponType.FIST,
          1,
          this,
          "#3e3846",
          this.enemySize,
        ...
        this.enemyRightWeapon = new Weapon(
          isLeftWeapon ? WeaponType.SHIELD : randomNumbOnRange(2, maxValue),
          -1,
          this,
          "#3e3846",
          this.enemySize,
        ...
        this.enemyLeftWeapon = new Weapon(
          isLeftWeapon ? randomNumbOnRange(2, maxValue) : WeaponType.SHIELD,
          1,
          this,
          "#3e3846",
          this.enemySize,
    ...
    this.rightWeaponActivationRange = this.getWeaponDistance(
      this.enemyRightWeapon,
    );
    this.leftWeaponActivationRange = this.getWeaponDistance(
      this.enemyLeftWeapon,
    );

    this.priorityWeapon =
      this.enemyRightWeapon.weaponType === WeaponType.FIST ||
      this.enemyRightWeapon.weaponType === WeaponType.SHIELD
        ? this.enemyLeftWeapon
        : this.enemyRightWeapon;
  }
  ...
  getWeaponDistance(weapon) {
    return (
      toPixelSize(weapon.sprite.length * weapon.size) *
  ...
  getEnemyLife() {
    if (GameVars.gameLevel < 3) {
      ...
      return this.enemyType === EnemyType.BASIC
        ? randomNumbOnRange(1, 2)
        : randomNumbOnRange(8, 10);
    } else {
      return this.enemyType === EnemyType.BASIC
        ? randomNumbOnRange(2, 3)
        : randomNumbOnRange(12, 14);
    }
  }
      ...
      if (this.isAlive) {
        ...
        this.div
          ...
          .finished.then(() => {
            if (this.enemyType === EnemyType.BOSS) {
              for (let i = randomNumbOnRange(1, 2); i > 0; i--) {
                this.room.items.push(
                  new Item(
                    GameVars.gameW / 2 +
                      toPixelSize(randomNumbOnRange(-32, 32)),
                    GameVars.gameH / 2 +
                      toPixelSize(randomNumbOnRange(-32, 32)),
                    ItemType.HEART,
                    null,
            ...
            if (
              randomNumb(100) < 10 &&
              this.priorityWeapon.weaponType !== WeaponType.FIST
            ) {
              ...
              GameVars.currentRoom.items.push(
                new Item(
                  ...
                  this.collisionObj.y,
                  ItemType.WEAPON,
                  this.priorityWeapon.weaponType,
  ...
  handleInput() {
    ...
    while (
      this.room.enemies.find(
        (enemy) =>
          enemy !== this &&
          circleToCircleCollision(this.targetPos, enemy.collisionObj),
      ) ||
      this.room.stonesBlocks.find((stone) =>
        rectCircleCollision(this.targetPos, stone.collisionObj),
      ) ||
      this.room.spikesBlocks.find((spike) =>
        rectCircleCollision(this.targetPos, spike.collisionObj),
      )
    ) {
      this.setTargetPosBasedOnEnemySubType();
      forceBreakCounter++;
    ...
    if (xDistance < -toPixelSize(4) || xDistance > toPixelSize(4)) {
      xDistance > 0
        ? this.enemyKeys.set("d", true)
        : this.enemyKeys.set("a", true);
    }

    if (yDistance < -toPixelSize(4) || yDistance > toPixelSize(4)) {
      yDistance > 0
        ? this.enemyKeys.set("s", true)
        : this.enemyKeys.set("w", true);
    }

    if (this.enemyKeys.size > 0 && this.walkAnim.playState === "finished") {
      this.walkAnim = this.enemyCanv.animate(walk(), { duration: 160 });
      !this.enemyLeftWeapon.isPerformingAction &&
        this.enemyLeftWeapon.weaponCanv.animate(weaponWalkLeft(), {
          duration: 160,
        });
      !this.enemyRightWeapon.isPerformingAction &&
        this.enemyRightWeapon.weaponCanv.animate(weaponWalkRight(), {
          duration: 160,
        });
      GameVars.sound.walkSound();
    }

    const distance = toPixelSize(
      this.enemyKeys.size > 1 ? this.enemySpeed / 1.4142 : this.enemySpeed,
    );
    if (this.enemyKeys.has("d")) {
      newRectX += distance;
    }
    if (this.enemyKeys.has("a")) {
      newRectX -= distance;
    }
    if (this.enemyKeys.has("w")) {
      newRectY -= distance;
    }
    if (this.enemyKeys.has("s")) {
      newRectY += distance;
    }
  ...
  setTargetPosBasedOnEnemySubType() {
    switch (this.enemySubType) {
      case EnemySubType.AGRESSIVE:
        if (
          distBetwenObjs(this.collisionObj, GameVars.player.collisionObj) <
          toPixelSize(this.activationDistance)
        ) {
          if (
            circleToCircleCollision(this.collisionObj, this.targetPos) ||
            this.movTimeElapsed / 0.4 >= 1
          ) {
      ...
      case EnemySubType.AFRAID:
        if (this.movTimeElapsed / 0.6 >= 1) {
          if (
            ...
            let xDiff = Math.round(
              this.collisionObj.x - GameVars.player.collisionObj.x,
            );
            let yDiff = Math.round(
              this.collisionObj.y - GameVars.player.collisionObj.y,
            );

            this.targetPos.x =
              this.collisionObj.x +
              (Math.abs(xDiff) < toPixelSize(20)
                ? randomNumbOnRange(
                    -this.activationDistance * 2,
                    this.activationDistance * 2,
                  )
                : xDiff);
            this.targetPos.y =
              this.collisionObj.y +
              (Math.abs(yDiff) < toPixelSize(20)
                ? randomNumbOnRange(
                    -this.activationDistance * 2,
                    this.activationDistance * 2,
                  )
  ...
  setTargetPosWeaponBased(distance) {
    ...
    switch (this.priorityWeapon.weaponType) {
      ...
      case WeaponType.SPEAR:
        ...
        this.targetPos.y += toPixelSize(
          randomNumbOnRange(-distance, -distance / 8),
        );
        break;
      ...
      case WeaponType.MORNING_STAR:
        this.targetPos.x += toPixelSize(
          this.priorityWeapon.handDir > 0
            ? randomNumbOnRange(-distance, -distance / 8)
            : randomNumbOnRange(distance / 8, distance),
        );
  ...
  move(circle) {
    ...
    this.div.style.translate =
      this.collisionObj.x -
      (knight[0].length * toPixelSize(this.enemySize)) / 2 +
      "px " +
      (this.collisionObj.y -
        ((knight.length * toPixelSize(this.enemySize)) / 4) * 3) +
      "px";
  }

  atk() {
    distBetwenObjs(GameVars.player.collisionObj, this.collisionObj) <
      this.rightWeaponActivationRange &&
      randomNumb(100) < this.atkProbablility() &&
      this.enemyRightWeapon.action();
    distBetwenObjs(GameVars.player.collisionObj, this.collisionObj) <
      this.leftWeaponActivationRange &&
      randomNumb(100) < this.atkProbablility() &&
  ...
  draw() {
    genSmallBox(
      ...
      6,
      5,
      toPixelSize(this.enemySize),
      "#00000033",
      "#00000033",
    );
    drawSprite(this.enemyCanv, knight, toPixelSize(this.enemySize), 0, 0, {
      hd: "#999a9e",
      hl: "#686b7a",
      cm: this.enemySubType,
    });
