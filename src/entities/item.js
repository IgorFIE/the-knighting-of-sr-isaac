...
export class Item {
  ...
  init(x, y) {
    ...
    this.collisionObj = new CircleObject(
      ...
      (this.sprite[0].length > this.sprite.length
        ? this.sprite[0].length
        : this.sprite.length) * this.size,
    );
    ...
    this.shadowCanv.style.translate =
      -this.size +
      "px " +
      (this.size * this.sprite.length - shadowSize / 2) +
      "px";

    setElemSize(
      this.itemCanv,
      this.sprite[0].length * this.size,
      this.sprite.length * this.size,
    );
    ...
    if (this.timeElapsed / 1 >= 1) {
      if (
        ...
        switch (this.itemType) {
          ...
          case ItemType.HEART:
            if (
              GameVars.player.lifeBar.life !== GameVars.player.lifeBar.totalLife
            ) {
  ...
  validateMovement(x, y) {
    ...
    this.itemDiv.style.translate =
      this.collisionObj.x -
      (this.sprite[0].length * this.size) / 2 +
      "px " +
      (this.collisionObj.y - (this.sprite.length * this.size) / 2) +
      "px";
  }
