...
export class GameBoard {
  ...
  canPlaceSpecialRoom(x, y) {
    if (this.board[y][x] == null) {
      ...
      return (
        (countSides == 1 || countSides == 2) &&
        !this.containsSpecialRoomArround(x, y)
      );
    }
    return false;
  }
  ...
  containsSpecialRoomArround(startX, startY) {
    for (let y = -1; y <= 1; y++) {
      for (let x = -1; x <= 1; x++) {
        if (
          startX + x >= 0 &&
          startX + x < this.board.length &&
          startY + y >= 0 &&
          startY + y < this.board.length &&
  ...
  createPaths() {
    ...
    this.rooms.forEach((room) => {
      ...
      if (room.roomY - 1 >= 0 && this.board[room.roomY - 1][room.roomX]) {
        doorType = this.getDoorBasedOnRoomType(
          isSpecialRoom(this.board[room.roomY][room.roomX].roomType)
            ? this.board[room.roomY][room.roomX].roomType
            : this.board[room.roomY - 1][room.roomX].roomType,
        );
        room.setDoor(w / 2 - 3, w / 2 + 1, 0, 0, 0, -1, doorType);
        room.setDoor(w / 2 - 2, w / 2, 1, 1, 0, -1, doorType);
      }
      if (
        room.roomY + 1 < this.board.length &&
        this.board[room.roomY + 1][room.roomX]
      ) {
        doorType = this.getDoorBasedOnRoomType(
          isSpecialRoom(this.board[room.roomY][room.roomX].roomType)
            ? this.board[room.roomY][room.roomX].roomType
      ...
      if (room.roomX - 1 >= 0 && this.board[room.roomY][room.roomX - 1]) {
        doorType = this.getDoorBasedOnRoomType(
          isSpecialRoom(this.board[room.roomY][room.roomX].roomType)
            ? this.board[room.roomY][room.roomX].roomType
            : this.board[room.roomY][room.roomX - 1].roomType,
        );
        room.setDoor(0, 0, h / 2 - 3, h / 2 + 1, -1, 0, doorType);
        room.setDoor(1, 1, h / 2 - 2, h / 2, -1, 0, doorType);
      }
      if (
        room.roomX + 1 < this.board[0].length &&
        this.board[room.roomY][room.roomX + 1]
      ) {
        doorType = this.getDoorBasedOnRoomType(
          isSpecialRoom(this.board[room.roomY][room.roomX].roomType)
            ? this.board[room.roomY][room.roomX].roomType
