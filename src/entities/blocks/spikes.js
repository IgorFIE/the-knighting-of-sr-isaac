...
export class Spikes {
  constructor(room, blockRoomX, blockRoomY) {
    ...
    this.init(
      blockRoomX * toPixelSize(16) + toPixelSize(2),
      blockRoomY * toPixelSize(16) + toPixelSize(2),
    );
  }
  ...
  update() {
    if (this.elapsedTime / 0.25 >= 1) {
      ...
      if (
        rectCircleCollision(GameVars.player.collisionObj, this.collisionObj)
      ) {
    ...
    for (let y = -1; y < 6; y += 4) {
      for (let x = -1; x < 8; x += 5) {
        drawSprite(
          ...
          spike,
          toPixelSize(2),
          this.collisionObj.x / toPixelSize(2) + x,
          this.collisionObj.y / toPixelSize(2) + y,
        );
      }
    }
  }
