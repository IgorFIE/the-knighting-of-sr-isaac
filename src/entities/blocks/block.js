...
export class Block {
  constructor(room, blockRoomX, blockRoomY, blockType) {
    ...
    this.collisionObj = new SquareObject(
      blockRoomX * toPixelSize(16),
      blockRoomY * toPixelSize(16),
      toPixelSize(16),
      toPixelSize(16),
    );
  ...
  draw() {
    ...
    switch (this.blockType) {
      case BlockType.WALL:
        ...
        generateBox(
          this.room.roomCanv,
          convertToMapPixel(this.collisionObj.x),
          convertToMapPixel(this.collisionObj.y),
          convertToMapPixel(this.collisionObj.w - toPixelSize(2)),
          convertToMapPixel(this.collisionObj.h - toPixelSize(2)),
          toPixelSize(2),
          blockColors.dk,
          (x, y, endX, endY) => {
            return (
      ...
      case BlockType.DOOR_CLOSE:
        ...
        this.createDoor(doorCtx, () => {
          ...
          generateBox(
            ...
            (x, y, endX, endY) => {
              return (
                (y === 0 &&
                  this.blockRoomY !== 1 &&
                  this.blockRoomY < Math.round(GameVars.roomHeight) - 1) || // Top lines
                (x === endX &&
                  this.blockRoomX > 0 &&
                  this.blockRoomX !== Math.round(GameVars.roomWidth) - 2) || // right Lines
                (y < 3 && this.blockRoomY === 0) || // topDoorFrame
                (x > endX - 3 &&
                  this.blockRoomX === Math.round(GameVars.roomWidth) - 1)
              );
            },
          );
          generateBox(
            ...
            (x, y, endX, endY) => {
              return (
                (y === endY &&
                  this.blockRoomY > 0 &&
                  this.blockRoomY !== Math.round(GameVars.roomHeight) - 2) || //Bottom lines
                (x === 0 &&
                  (this.blockRoomX === 0 ||
                    this.blockRoomX === Math.round(GameVars.roomWidth) - 2 ||
                    (this.blockRoomX > 1 &&
                      this.blockRoomX < Math.round(GameVars.roomWidth) - 2))) || // left lines
                (y < 2 && this.blockRoomY === 0) || // top door frame
                (y > endY - 2 &&
                  this.blockRoomY === Math.round(GameVars.roomHeight) - 1) || // bottom door frame
                (x < 2 && this.blockRoomX === 0) || // left door frame
                (x > endX - 2 &&
                  this.blockRoomX === Math.round(GameVars.roomWidth) - 1) // right door frame
              );
            },
          );
          if (
            this.blockRoomY < 3 ||
            this.blockRoomY > Math.round(GameVars.roomHeight) - 3
          ) {
            ...
            this.createDoorFrame(
              doorCtx,
              this.collisionObj.x + toPixelSize(8),
              this.collisionObj.y,
              toPixelSize(8),
              toPixelSize(16),
            );
          ...
          if (this.doorType === DoorType.TREASURE) {
            if (
              (this.blockRoomX === 1 &&
                this.room.backBlocks[this.blockRoomY - 1][this.blockRoomX]
                  .blockType == BlockType.FLOOR &&
                this.room.backBlocks[this.blockRoomY + 1][this.blockRoomX]
                  .blockType == BlockType.FLOOR) ||
              (this.blockRoomX === Math.round(GameVars.roomWidth) - 1 &&
                this.room.backBlocks[this.blockRoomY - 2][this.blockRoomX]
                  .blockType == BlockType.FLOOR &&
                this.room.backBlocks[this.blockRoomY + 2][this.blockRoomX]
                  .blockType == BlockType.FLOOR)
            ) {
              this.createKeyHole(
                doorCtx,
                this.collisionObj.x,
                this.collisionObj.y + toPixelSize(4),
              );
            }
            if (
              (this.blockRoomY === 0 &&
                this.room.backBlocks[this.blockRoomY][this.blockRoomX - 2]
                  .blockType == BlockType.FLOOR &&
                this.room.backBlocks[this.blockRoomY][this.blockRoomX + 2]
                  .blockType == BlockType.FLOOR) ||
              (this.blockRoomY === Math.round(GameVars.roomHeight) - 2 &&
                this.room.backBlocks[this.blockRoomY][this.blockRoomX - 1]
                  .blockType == BlockType.FLOOR &&
                this.room.backBlocks[this.blockRoomY][this.blockRoomX + 1]
                  .blockType == BlockType.FLOOR)
            ) {
              this.createKeyHole(
                doorCtx,
                this.collisionObj.x + toPixelSize(6),
                this.collisionObj.y + toPixelSize(10),
              );
            }
          }
        });
  ...
  validateBlock(
    ...
    if (
      this.blockRoomX > 0 &&
      this.blockRoomX < GameVars.roomWidth - 1 &&
      this.blockRoomY > 0 &&
      this.blockRoomY < GameVars.roomHeight - 1
    ) {
  ...
  createDoor(ctx, elseFn) {
    if (
      (this.blockRoomX === this.room.backBlocks[0].length - 1 ||
        this.blockRoomX === 0) &&
      this.blockRoomY - 1 > 0 &&
      this.room.backBlocks[this.blockRoomY - 1][this.blockRoomX].blockType ==
        BlockType.WALL
    ) {
      createWallBlock(
        ctx,
        this.collisionObj.x,
        this.collisionObj.y - toPixelSize(8),
      );
      this.createDoorFrame(
        ctx,
        this.collisionObj.x,
      ...
      (this.blockRoomX === this.room.backBlocks[0].length - 1 ||
        this.blockRoomX === 0) &&
      this.blockRoomY + 1 < this.room.backBlocks.length &&
      this.room.backBlocks[this.blockRoomY + 1][this.blockRoomX].blockType ==
        BlockType.WALL
    ) {
      createWallBlock(
      ...
      (this.blockRoomY === this.room.backBlocks.length - 1 ||
        this.blockRoomY === 0) &&
      this.blockRoomX - 1 > 0 &&
      this.room.backBlocks[this.blockRoomY][this.blockRoomX - 1].blockType ==
        BlockType.WALL
    ) {
      createWallBlock(
        ctx,
      ...
      (this.blockRoomY === this.room.backBlocks.length - 1 ||
        this.blockRoomY === 0) &&
      this.blockRoomX + 1 < this.room.backBlocks[0].length &&
      this.room.backBlocks[this.blockRoomY][this.blockRoomX + 1].blockType ==
        BlockType.WALL
    ) {
      createWallBlock(
  ...
  createDoorFrame(ctx, x, y, w, h) {
    ...
    ctx.fillRect(
      x + toPixelSize(w / toPixelSize(2)),
      y + toPixelSize(h / toPixelSize(2) - 2),
      toPixelSize(2),
      toPixelSize(2),
    );
...
export const createFloorBlock = (canvas, x, y) => {
  ...
  generateBox(
    ...
    convertToMapPixel(x),
    convertToMapPixel(y),
    convertToMapPixel(toPixelSize(14)),
    convertToMapPixel(toPixelSize(14)),
    toPixelSize(2),
    floorColors.lt,
    () => randomNumb(100) < 5,
