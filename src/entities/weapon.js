...
export class Weapon {
  ...
  init() {
    setElemSize(
      this.weaponCanv,
      this.sprite[0].length * toPixelSize(this.size),
        ...
        return this.weaponCanv.animate(
          {
            transform: [
              "translateY(0)",
              "translateY( " + toPixelSize(this.size * 3) + "px)",
              "translateY(0)",
            ],
        ...
        return this.weaponCanv.animate(
          {
            transform: [
              ...
              "translateY( " + toPixelSize(this.size * 3) + "px)  scale(2)",
              "translateY(0) scale(1)",
            ],
            easing: ["ease-in", "ease-out", "ease-out"],
            offset: [0, 0.5, 1],
        ...
        return this.weaponCanv.animate(
          {
            transform: [
              "translateY(0)",
              "translateY( " + toPixelSize(this.size * 12) + "px)",
              "translateY(0)",
            ],
            easing: ["ease-in", "ease-out", "ease-out"],
            offset: [0, 0.5, 1],
        ...
        return this.weaponCanv.animate(
          {
            transform: [
              "translateY(0)",
              "translateY( " + toPixelSize(this.size * 4) + "px)",
              "translateY(0)",
            ],
            easing: ["ease-in", "ease-out", "ease-out"],
            offset: [0, 0.1, 1],
  ...
  getRelativePos() {
    switch (this.weaponType) {
      case WeaponType.FIST:
        return {
          x: toPixelSize(this.handDir === -1 ? -this.size : this.size * 2),
          y: toPixelSize(this.size * 4.5),
        };
      case WeaponType.SHIELD:
        return {
          x: toPixelSize(this.size * 2 * this.handDir),
          y: toPixelSize(this.size * 3),
        };
      case WeaponType.SWORD:
        return {
          x: toPixelSize(this.size * 3 * this.handDir),
          y: -toPixelSize(2),
          r: -90,
        };
      case WeaponType.GREATSWORD:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 10) : this.size * 8,
          ),
          y: -toPixelSize(
            this.handDir === -1 ? this.size * 0.5 : this.size * 5.5,
          ),
          r: 45 * this.handDir,
        };
      case WeaponType.TROWING_KNIVE:
      case WeaponType.SPEAR:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 2) : this.size * 4,
          ),
          y: -toPixelSize(this.size),
        };
      case WeaponType.HALBERD:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 4) : this.size * 4,
          ),
          y: -toPixelSize(this.size * 8),
          r: 0,
        };
      case WeaponType.HAMMER:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 4) : this.size * 9,
          ),
          y: -toPixelSize(
            this.handDir === -1 ? -this.size * 8 : -this.size * 6,
          ),
          r: 135 * this.handDir,
        };
      case WeaponType.AXE:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 6) : this.size * 7,
          ),
          y: -toPixelSize(this.handDir === -1 ? -this.size * 2 : 0),
          r: 45 * this.handDir,
        };
      case WeaponType.MORNING_STAR:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 5) : this.size * 6,
          ),
          y: -toPixelSize(
            this.handDir === -1 ? -this.size * 4 : -this.size * 2,
          ),
          r: 45 * this.handDir,
        };
      case WeaponType.CROSSBOW:
        return {
          ...
          y: toPixelSize(this.handDir === -1 ? this.size * 2 : -this.size * 3),
          r: 45 * this.handDir,
        };
      ...
      case WeaponType.TROWING_AXE:
        return {
          x: toPixelSize(
            this.handDir === -1 ? -(this.size * 6) : this.size * 8,
          ),
          y: toPixelSize(this.handDir === -1 ? this.size * 2 : this.size * 1),
          r: 45 * this.handDir,
        };
    }
  }
  ...
  setWeaponPos() {
    this.weaponDiv.style.translate =
      this.relativePos.x + "px " + this.relativePos.y + "px";
    switch (this.weaponType) {
      case WeaponType.SWORD:
  ...
  getWeaponAtkLine() {
    switch (this.weaponType) {
        ...
        return [
          {
            ...
            y: this.relativePos.y + this.sprite.length * toPixelSize(this.size),
          },
          {
            x:
              this.relativePos.x +
        ...
        return [
          {
            ...
            y:
              this.relativePos.y +
              (90 * this.sprite.length * toPixelSize(this.size)) / 100,
          },
        ];
      case WeaponType.GREATSWORD:
      case WeaponType.HAMMER:
      ...
      case WeaponType.SPEAR:
        return [
          {
            x: this.relativePos.x + toPixelSize(this.size / 2),
            y:
              this.relativePos.y +
              (this.sprite.length * toPixelSize(this.size)) / 2,
          },
  ...
  getUpdatedWeaponAtkLine(box, transform) {
    switch (this.weaponType) {
      case WeaponType.FIST:
        return [
          {
            x: box.x + this.atkLine[0].x,
            y: box.y + this.atkLine[0].y + transform.f * 1.5,
          },
          {
            x: box.x + this.atkLine[1].x,
            y: box.y + this.atkLine[1].y + transform.f * 1.5,
          },
        ];
      case WeaponType.SHIELD:
        return [
          {
            x:
              box.x +
              this.atkLine[0].x -
              toPixelSize(this.size) * (transform.m11 - 1),
            y: box.y + this.atkLine[0].y + transform.f * transform.m11,
          },
          {
            x:
              box.x +
              this.atkLine[1].x +
              toPixelSize(this.size) * (transform.m11 - 1),
            y: box.y + this.atkLine[1].y + transform.f * transform.m11,
          },
        ];
      case WeaponType.SWORD:
      ...
      case WeaponType.SPEAR:
        return [
          {
            x: box.x + this.atkLine[0].x,
            y: box.y + this.atkLine[0].y + transform.f * 1.1,
          },
          {
            x: box.x + this.atkLine[1].x,
            y: box.y + this.atkLine[1].y + transform.f * 1.1,
          },
        ];
    }
  }
  ...
  update() {
    ...
    if (this.isPerformingAction && !isProjectileWeapon(this.weaponType)) {
      let transform = new WebKitCSSMatrix(
        window.getComputedStyle(this.weaponCanv).transform,
      );
      let newAtkLine = this.getUpdatedWeaponAtkLine(
        this.parentDiv.getBoundingClientRect(),
        transform,
      );
      ...
      if (this.isPlayer) {
        GameVars.currentRoom.enemies.forEach(
          (enemy) =>
            lineCircleCollision(newAtkLine, enemy.collisionObj) &&
            this.dealDmgToBlock(enemy),
        );
      } else {
        lineCircleCollision(newAtkLine, GameVars.player.collisionObj) &&
          this.dealDmgToBlock(GameVars.player);
      }
    }
    // }
  }
  ...
  createProjectile() {
    ...
    GameVars.currentRoom.projectiles.push(
      new Projectile(
        ...
        this.color,
        this.size,
        getProjectileType(this.weaponType),
        this.isPlayer,
      ),
    );
  }
