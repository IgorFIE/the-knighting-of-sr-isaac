...
const createGameElements = () => {
  ...
  gameOverCanv = createElem(
    ...
    GameVars.isMobile,
    "#ff4b4be6",
    () => skipElapsedTime / 1 >= 1 && skipGameOver(),
  );
...
const drawMainMenu = () => {
  ...
  let wKnightCenter =
    (GameVars.gameW % 2 === 0 ? GameVars.gameW : GameVars.gameW + 1) /
    toPixelSize(10) /
    2;
  let hKnightCenter =
    (GameVars.gameH % 2 === 0 ? GameVars.gameH : GameVars.gameH + 1) /
    toPixelSize(10) /
    2;
  ...
  drawSprite(
    ...
    leftSprite,
    toPixelSize(10),
    wKnightCenter - 4 - leftSprite[0].length / 2,
    hKnightCenter - 1.5,
    { wc: "#cd9722" },
  );
  drawSprite(
    ...
    rightSprite,
    toPixelSize(10),
    wKnightCenter + 4 - rightSprite[0].length / 2,
    hKnightCenter - 1.5,
    { wc: "#cd9722" },
  );
  ...
  genSmallBox(
    ...
    -1,
    -1,
    Math.floor(mainMenuCanv.width / toPixelSize(2)) + 2,
    32,
    toPixelSize(2),
    "#060606",
    "#060606",
  ...
  drawPixelTextInCanvas(
    convertTextToPixelArt("Knight Arena"),
    mainMenuCanv,
    toPixelSize(3),
    Math.round(GameVars.gameW / 2 / toPixelSize(3)),
    11,
    "#edeef7",
    1,
  );
  ...
  genSmallBox(
    mainMenuCanv,
    -1,
    Math.floor(mainMenuCanv.height / toPixelSize(2)) - 16,
    Math.floor(mainMenuCanv.width / toPixelSize(2)) + 2,
    17,
    toPixelSize(2),
    "#060606",
  ...
  drawPixelTextInCanvas(
    convertTextToPixelArt("each weapon has a different atk pattern"),
    mainMenuCanv,
    toPixelSize(1),
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels - 24,
    "#edeef7",
    1,
  );
  drawPixelTextInCanvas(
...
const drawBtn = (btn, xPos, topText, bottomText) => {
  btn.style.translate =
    ...
    xPos +
    "px " +
    (mainMenuCanv.height - toPixelSize(36) - btn.height) +
    "px";
  genSmallBox(btn, 0, 0, 32, 14, toPixelSize(2), "#060606", "#060606");
  drawPixelTextInCanvas(
    convertTextToPixelArt(topText + "start game"),
    btn,
    toPixelSize(1),
    35,
    10,
  ...
  drawPixelTextInCanvas(
    convertTextToPixelArt(bottomText),
    btn,
    toPixelSize(1),
    33,
    20,
...
const drawSoundBtn = (isResize) => {
  ...
  if (lastSoundState !== isSoundOn || isResize) {
    ...
    soundBtnCanv
      .getContext("2d")
      .clearRect(0, 0, soundBtnCanv.width, soundBtnCanv.height);
    genSmallBox(
      soundBtnCanv,
      0,
      0,
      ...
      11,
      toPixelSize(1),
      isSoundOn ? "#00000066" : "#ffffffaa",
      isSoundOn ? "#100f0f66" : "#ffffff66",
    );
    drawSprite(soundBtnCanv, speaker, toPixelSize(1), 10, 3);
    drawPixelTextInCanvas(
      convertTextToPixelArt("m"),
      soundBtnCanv,
      toPixelSize(1),
      6,
      6,
...
const drawScore = (isResize) => {
  if (lastScore != GameVars.score || isResize) {
    scoreCanv.style.translate =
      toPixelSize(12) + "px " + toPixelSize(10) + "px";
    const textArray = convertTextToPixelArt(
      !GameVars.game
        ? "top score - " + window.highscores.getScore()
...
const drawGameOver = () => {
  gameOverCanv
    .getContext("2d")
    .clearRect(0, 0, gameOverCanv.width, gameOverCanv.height);
  genSmallBox(
    gameOverCanv,
    -20,
    GameVars.gameHgAsPixels / 2 - 85,
    GameVars.gameWdAsPixels + 40,
    180,
    GameVars.pixelSize,
    "black",
    "white",
  ...
  drawPixelTextInCanvas(
    convertTextToPixelArt("Game over"),
    gameOverCanv,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 - 40,
    "black",
    5,
  );
...
const drawScoreCalc = (canvas) => {
  drawPixelTextInCanvas(
    convertTextToPixelArt("score"),
    canvas,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 - 10,
    "black",
    4,
  );
  drawPixelTextInCanvas(
    convertTextToPixelArt("enemies killed - " + GameVars.enemyKills * 10),
    canvas,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 + 15,
    "black",
    2,
  );
  drawPixelTextInCanvas(
    convertTextToPixelArt(
      "boss enemies killed - " + GameVars.enemyBossKills * 100,
    ),
    canvas,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 + 30,
    "black",
    2,
  );
  drawPixelTextInCanvas(
    convertTextToPixelArt("keys caught - " + GameVars.keyCaught * 50),
    canvas,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 + 45,
    "black",
    2,
  );
  drawPixelTextInCanvas(
    convertTextToPixelArt("level - " + (GameVars.gameLevel - 1) * 500),
    canvas,
    GameVars.pixelSize,
    GameVars.gameWdAsPixels / 2,
    GameVars.gameHgAsPixels / 2 + 60,
    "black",
    2,
  );
  drawPixelTextInCanvas(
    convertTextToPixelArt(
      (GameVars.score > GameVars.highScore ? "new record!" : "total") +
        " - " +
        GameVars.score,
    ),
    canvas,
...
const startGame = (weaponType, handir) => {
  ...
  GameVars.resetGameVars();
  skipElapsedTime = 0;
  (handir < 0 && (GameVars.lastPlayerRightWeaponType = weaponType)) ||
...
const updateScore = () => {
  ...
  GameVars.score =
    GameVars.enemyKills * 10 +
    GameVars.keyCaught * 50 +
    GameVars.enemyBossKills * 100 +
    (GameVars.gameLevel - 1) * 500;
};
